#!/usr/bin/env python3
"""Multi-sport Forebet scraper - Tennis, Basketball, Hockey, Volleyball, Handball, Cricket, Baseball"""
...
SPORTS = [
    ("tennis", "Tennis", "https://www.forebet.com/fr/tennis/predictions/{date}"),
    ("basketball", "Basketball", "https://www.forebet.com/fr/basketball/predictions/{date}"),
    ("hockey", "Hockey", "https://www.forebet.com/fr/hockey/predictions/{date}"),
    ("volleyball", "Volleyball", "https://www.forebet.com/fr/volleyball/predictions/{date}"),
    ("handball", "Handball", "https://www.forebet.com/fr/handball/predictions/{date}"),
    ("cricket", "Cricket", "https://www.forebet.com/fr/cricket/predictions/{date}"),
    ("baseball", "Baseball", "https://www.forebet.com/fr/baseball/predictions/{date}"),
]
...
def fetch_page(url):
    try:
        ...
        d = r.json()
        return d["solution"]["response"] if d.get("status")=="ok" else None
    except:
        return None
...
def parse_rcnt(rcnt, date_str, sport):
    ...
    if fprc:
        ...
        if len(spans) >= 2:
            ...
            if len(spans) >= 3: m["prob_c"] = int(spans[2].get_text(strip=True))
    pred_div = rcnt.find("div", class_="predict_y") or rcnt.find("div", class_="predict_no") or rcnt.find("div", class_="predict_e")
    if pred_div:
    ...
    if wth:
        ...
        if wn: m["weather"] = wn.get_text(strip=True)
...
def main():
    ...
    start, end = datetime(2026,3,1), datetime(2026,3,26)
    all_matches = []
    current = start
    total = len(SPORTS) * ((end-start).days+1)
    n = 0
    print(f"Multi-sport: {len(SPORTS)} sports × 26 days = {total} requests\n")
    while current <= end:
        ds = current.strftime("%Y-%m-%d")
        print(f"📅 {ds}")
        for slug, name, url_tpl in SPORTS:
            n += 1
            ...
            if html:
                ...
                for schema in soup.find_all("div", class_="schema"):
                    for rcnt in schema.find_all("div", class_="rcnt"):
                        ...
                        if m: all_matches.append(m); cnt += 1
            print(f"  [{n}/{total}] {name:15s} -> {cnt} matchs")
            time.sleep(3)
        current += timedelta(days=1)
    if all_matches:
        df = pd.DataFrame(all_matches)
        csv = f"{OUTPUT_DIR}/multi_sport_2026-03.csv"
        df.to_csv(csv, index=False)
        print(f"\n{'='*50}")
        print(f"✅ {len(df)} total rows saved")
        for _, name, _ in SPORTS:
            n = len(df[df['sport']==name])
            print(f"   {name:15s} : {n} rows")
    else:
        print("❌ No matches!")
